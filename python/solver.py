...
grid_magic_cells = np.array(
    list(
        "3..b...3...b..3.2...c...c...2...2...b.b...2..b..2...b...2..b....2.....2.....c...c...c...c...b...b.b...b..3..b...2...b..3..b...b.b...b...c...c...c...c.....2.....2....b..2...b...2..b..2...b.b...2...2...c...c...2.3..b...3...b..3"
    )
).reshape(15, 15)
# 2 = mot x2
# 3 = mot x3
        ...
        for letter in hand:
            for position in positions:
                ...
                if direction == Direction.LR:
                    if (
                        1 <= start_pos[1]
                        and c_grid[p := (start_pos[0], start_pos[1] - 1)] == "."
                    ):  # peut on aller à gauche du mot actuel ?
                        new_positions.append(p)
                    if (
                        end_pos[1] <= grid.shape[1] - 2
                        and c_grid[p := (end_pos[0], end_pos[1] + 1)]
                    ):  # à droite ?
                        new_positions.append(p)
                else:  # UD
                    if (
                        1 <= start_pos[0]
                        and c_grid[p := (start_pos[0] - 1, start_pos[1])] == "."
                    ):  # en haut ?
                        new_positions.append(p)
                    if (
                        end_pos[0] <= grid.shape[0] - 2
                        and c_grid[p := (end_pos[0] + 1, end_pos[1])]
                    ):  # en bas ?
                        new_positions.append(p)
...
def compute_score_and_pulled_letters(
    grid, word, position, direction, hand_size=7, return_pulled_letters=False
):
    word_multiplier = 1
    word_score = 0
    total_score = 0
    ...
    for k, letter in enumerate(word):
        ...
        if grid[position] == ".":
            ...
            ortho_score = _letter_score(letter) * (
                2 if magic == "b" else (3 if magic == "c" else 1)
            )
            there_is_an_ortho_word = (
                False  # do not count this score if it's actually just one letter!
            )
            if direction == Direction.LR:
                # on doit checker ORTHOGONALEMENT
                for i in range(position[0] - 1, -1, -1):
    ...
    if len(pulled_letters) == hand_size:
        total_score += 50
...
def solve(grid: np.ndarray, hand: list[str], dictionary: set) -> list[Solution]:
    ...
    for dot_row, dot_col in zip(*dot_indices):
        ...
        for dr, dc in _directions:
            ...
            if (
                0 <= adj_row < grid.shape[0]
                and 0 <= adj_col < grid.shape[1]
                and grid[adj_row, adj_col].isalpha()
            ):
    ...
    for word, position, direction in matches:
        solutions.append(
            Solution(
                ...
                position,
                direction,
                *compute_score_and_pulled_letters(grid, word, position, direction),
            )
        )
